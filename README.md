...
Sets the list of file extensions that will be ignored. This applies to HTML, JavaScript, and CSS files.
...
Sets the ignored JavaScript starts with array. Any script tag with an `src` attribute starting with this will be ignored. An error is thrown if this is not an array.
...
Sets the ignored JavaScript contains array. Any script tag with an `src` attribute containing this will be ignored. An error is thrown if this is not an array.

### get ignoreCssContains()
See `get ignoreJsContains()`.

### set ignoreCssContains(array)
See `set ignoreJsContains()`.
...
Sets the additional JavaScript position. An error is thrown if the position is not either `before` or `after`.

### get includeCssPosition()
See `get includeJsPosition()`.

### set includeCssPosition(position)
See `set includeJsPosition()`.

### get uglifyJsOptions()
Returns the options that will be passed to [UglifyJS](https://www.npmjs.com/package/uglify-js).

### set uglifyJsOptions(options)
Sets the options that will be passed to UglifyJS. An error is thrown if the options are not an object. Additional errors may be thrown by UglifyJS.
